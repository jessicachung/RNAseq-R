...
author: "Belinda Phipson, Anna Trigos, Matt Ritchie, Maria Doyle, Harriet Dashnow, Charity Law, Jessica Chung"
date: "June 2019"
output: 
  html_document:
    toc: true
...
Relative log expression (RLE) plots are another tool for visualising unwanted variation in high-dimensional expression data [@Gandolfo2018]. We can assess if normalisation has been successful using RLE plots. 
...
When there is a lot of differential expression, sometimes we may want to cut-off on a fold change threshold as well as a p-value threshold so that we follow up on the most biologically significant genes. However, it is not recommended to simply rank by p-value and then discard genes with small logFC's, as this has been shown to increase the false discovery rate. In other words, you are not controlling the false discovery rate at 5\% any more. There is a function called `treat` in the *limma* package that performs this style of analysis correctly [@McCarthy2009]. `treat` will simply take our `fit.cont` object, as well as a user-specified log fold change cut-off, and recalculate the moderated t-statistics and p-values with the new information about logFC.
...
Other databases of gene sets that are available come from the Broad Institute's Molecular Signatures Database ([MSigDB](http://software.broadinstitute.org/gsea/msigdb)). CAMERA is good option for testing a very large number of gene sets such as the MSigDB sets, as it is very fast. CAMERA is known as a competitive gene set test, however it has the advantage that it can take into account inter-gene correlation within each gene set [@wu2012camera]. It also works seamlessly with a `voom` object, taking into account the mean-variance relationship in RNA-Seq data.
...
Let's have a look at the top pathway in the CAMERA test that you did previously with the Hallmark gene sets, "HALLMARK_MYC_TARGETS_V2".
...
barcodeplot(fit.cont$coeff[,1], index=H.ind[["HALLMARK_MYC_TARGETS_V2"]], main="LogFC: HALLMARK_MYC_TARGETS_V2")
# barcode plot using t-statistics
barcodeplot(fit.cont$t[,1], index=H.ind[["HALLMARK_MYC_TARGETS_V2"]], main="T-statistic: HALLMARK_MYC_TARGETS_V2")
```
...
> 1. Try producing a barcode plot with two gene sets on one plot. For example, gene sets "LABBE_WNT3A_TARGETS_UP" (Up-regulated genes in mammary epithelium after stimulation with WNT3A) and "LABBE_WNT3A_TARGETS_DN" (Down-regulated genes in mammary epithelium after stimulation with WNT3A). You can put two gene sets on one plot by adding a gene set to the `index2` argument.
>

**Solution**
```{r,echo=FALSE}
# Solution 1
barcodeplot(fit.cont$coeff[,2], index=H.ind[["HALLMARK_MYC_TARGETS_V2"]], main="L.PregVsLac: HALLMARK_MYC_TARGETS_V2")
# Solution 2
